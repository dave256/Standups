...
class RecordMeetingModel: Hashable, ObservableObject {
    ...
    let standup: Standup
    var parentModel: RecordingPresenter?
    ...
    init(
        ...
        standup: Standup,
        parentModel: RecordingPresenter?
    ) {
    ...
    func alertButtonTapped(_ action: AlertAction?) async {
        switch action {
        case .confirmSave?:
            await parentModel?.meetingFinished(transcript: self.transcript)
        case .confirmDiscard?:
            parentModel?.discardMeeting()
        case nil:
            break
        }
    }
...
struct RecordMeeting_Previews: PreviewProvider {
    static var previews: some View {
        NavigationStack {
            RecordMeetingView(
                model: RecordMeetingModel(standup: .mock, parentModel: nil)
            )
        }
            ...
            NavigationStack {
                RecordMeetingView(
                    model: withDependencies {
                        ...
                        RecordMeetingModel(standup: .mock, parentModel: nil)
                    }
                )
            }
        }
